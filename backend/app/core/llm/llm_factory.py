...
class LLMFactory:
    """LLM 工厂类，根据配置创建各 Agent 的 LLM 实例。"""
    ...
    def get_all_llms(self) -> tuple[LLM, LLM, LLM, LLM, LLM]:
        ...
        Returns:
            包含 (coordinator_llm, modeler_llm, coder_llm, writer_llm, reviewer_llm) 的元组。
        """
        coordinator_llm = LLM(
        ...
        reviewer_llm = LLM(
            api_type=settings.REVIEWER_API_TYPE,
        ...
        return coordinator_llm, modeler_llm, coder_llm, writer_llm, reviewer_llm
